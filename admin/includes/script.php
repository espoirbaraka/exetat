<?php
  $noscours = array();
  $nosnombres = array();
  $stmt1 = $conn->prepare("SELECT CodeCours,SUBSTRING(Cours, 1, 15) as Cours FROM t_cours");
  $stmt1->execute();
  foreach($stmt1 as $row)
  {
    $cour = $row['CodeCours'];
    $cours = $row['Cours'];
    $stmt2 = $conn->prepare("SELECT COUNT(CodeQuestion) as nbre FROM t_question WHERE CodeCours=:cours");
    $stmt2->execute(['cours'=>$cour]);
    foreach($stmt2 as $row)
    {
      $nombres = $row['nbre'];
    }
    array_push($nosnombres, $nombres);
    array_push($noscours, $cours);
  }
